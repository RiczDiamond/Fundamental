<?php
    ...
    if (!function_exists('mol_unslash')) {
        /**
         * Verwijder slashes op strings of recursief op arrays.
         */
        function mol_unslash(mixed $value): mixed {
            if (is_array($value)) {
                $result = [];
                foreach ($value as $key => $item) {
                    $result[$key] = mol_unslash($item);
                }
                return $result;
            }
    ...
    if (!function_exists('sanitize_email')) {
        function sanitize_email(mixed $email): string {
            ...
            $clean = filter_var((string) mol_unslash($email), FILTER_SANITIZE_EMAIL);
    ...
    if (!function_exists('mol_create_nonce')) {
        function mol_create_nonce(string $action = '-1'): string {
            if (session_status() !== PHP_SESSION_ACTIVE) {
                session_start();
            }

            if (!isset($_SESSION['_mol_nonce']) || !is_array($_SESSION['_mol_nonce'])) {
                $_SESSION['_mol_nonce'] = [];
            }

            $bucket = $_SESSION['_mol_nonce'];
    ...
    if (!function_exists('mol_verify_nonce')) {
        function mol_verify_nonce(mixed $nonce, string $action = '-1'): bool {
            ...
            $entry = $_SESSION['_mol_nonce'][$action] ?? null;
            if (!is_array($entry)) {
                return false;
            }
            ...
            if ((time() - $createdAt) > 7200) {
                unset($_SESSION['_mol_nonce'][$action]);
                return false;
            }
    ...
    if (!function_exists('mol_nonce_field')) {
        function mol_nonce_field(string $action = '-1', string $name = '_wpnonce'): void {
            $token = mol_create_nonce($action);
            echo '<input type="hidden" name="' . esc_attr($name) . '" value="' . esc_attr($token) . '">';
        }
    }

    if (!function_exists('mol_require_valid_nonce')) {
        /**
         ...
         */
        function mol_require_valid_nonce(string $action, string $name = '_wpnonce', ?array $source = null): bool {
            $source = is_array($source) ? $source : $_POST;
            return mol_verify_nonce($source[$name] ?? '', $action);
        }
    }
    ...
    if (!function_exists('mol_redirect')) {
        /**
         * Redirect helper.
         */
        function mol_redirect(string $location, int $status = 302): void {
            header('Location: ' . $location, true, $status);
            exit;
        }
    }

    if (!function_exists('mol_safe_redirect')) {
        /**
         * Redirect alleen naar huidige host of relatieve URL.
         */
        function mol_safe_redirect(string $location, int $status = 302): void {
            $location = trim($location);

            if ($location === '') {
                mol_redirect(home_url('/'), $status);
            }

            if ($location[0] === '/' || $location[0] === '?' || $location[0] === '#') {
                mol_redirect(home_url($location), $status);
            }
